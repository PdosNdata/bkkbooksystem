...
import { Search, Plus, Package, Loader2, Calendar, Truck, CheckCircle, Save, Eye, BookOpen, Edit, Trash2 } from 'lucide-react'
...
export default function BookReceiptsPageNew1() {
  ...
  const handleDelete = async (receipt) => {
    const result = await Swal.fire({
      title: 'ยืนยันการลบ?',
      html: `คุณต้องการลบใบรับหนังสือ<br/><strong>${receipt.receipt_number}</strong><br/>ครั้งที่ ${receipt.delivery_number} ของชั้น ${gradeLabel[receipt.grade]}?`,
      icon: 'warning',
    ...
    try {
      // ลด received_quantity ใน order_items ก่อน
          ...
          if (orderItemsData) {
            ...
            for (const item of receipt.book_receipt_items) {
              ...
              for (const orderItem of relatedOrderItems) {
                ...
                if (toDeduct > 0) {
                  const newReceived = Math.max(0, currentReceived - toDeduct)
      ...
      Swal.fire({
        icon: 'success',
        title: 'ลบสำเร็จ',
        text: 'ลบใบรับหนังสือเรียบร้อยแล้ว',
        confirmButtonColor: '#2563eb'
      })
