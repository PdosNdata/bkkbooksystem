...
export default function WithdrawalsPage9() {
  const { user } = useAuth()
  ...
  const [showEditModal, setShowEditModal] = useState(false) // เพิ่ม modal แก้ไข
  const [selectedOrder, setSelectedOrder] = useState(null)
  const [selectedWithdrawal, setSelectedWithdrawal] = useState(null)
  const [withdrawItems, setWithdrawItems] = useState({})
  const [editingItems, setEditingItems] = useState([]) // รายการที่กำลังแก้ไข
  ...
  const [nextWithdrawalNumber, setNextWithdrawalNumber] = useState('') // เพิ่ม state สำหรับแสดงเลขที่ถัดไป
  ...
  const fetchData = async () => {
    ...
    console.log('Fetched withdrawals:', wData) // Debug
    console.log('Fetched orders:', validOrders) // Debug
  ...
  const handleDeleteItem = async (itemId, itemIndex) => {
    const result = await Swal.fire({
      title: 'ลบรายการนี้?',
      text: 'คุณต้องการลบรายการหนังสือนี้ออกจากใบเบิกใช่หรือไม่?',
      icon: 'warning',
      ...
      if (item.approved > 0) {
        try {
          ...
          if (stockFetchError) {
            ...
            stockUpdateErrors.push(`ไม่สามารถดึงข้อมูล stock สำหรับหนังสือ ID: ${bookId}`)
            continue
          }
          ...
          if (stockData) {
            ...
            if (stockUpdateError) {
              ...
              stockUpdateErrors.push(`ไม่สามารถอัปเดต stock สำหรับหนังสือ ID: ${bookId}`)
            }
          } else {
            console.warn(`ไม่พบข้อมูล stock สำหรับหนังสือ ID: ${bookId}, ชั้น: ${selectedGrade}, ปี: ${currentYear}`)
          }
        } catch (err) {
          console.error('Stock update error:', err)
          stockUpdateErrors.push(`เกิดข้อผิดพลาดในการอัปเดต stock: ${err.message}`)
        }
      }
    }

    // แสดงผลการอัปเดต stock
    if (stockUpdateErrors.length > 0) {
      console.warn('Stock update warnings:', stockUpdateErrors)
      // ยังคงแสดงว่าสร้างใบเบิกสำเร็จ แต่มีคำเตือนเรื่อง stock
      await Swal.fire({ 
        icon: 'warning', 
        title: 'สร้างใบเบิกสำเร็จ', 
        html: `เลขที่: ${withdrawalNumber}<br><small class="text-gray-500">หมายเหตุ: บางรายการอาจไม่สามารถอัปเดต stock ได้</small>`,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* New Withdrawal Modal - แสดงเลขที่ถัดไป */}
