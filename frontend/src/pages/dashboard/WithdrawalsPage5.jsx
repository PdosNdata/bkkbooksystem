...
import { useAuth } from '../../context/AuthContext' // เพิ่ม import
...
export default function WithdrawalsPage5() {
  ...
  const [officers, setOfficers] = useState([]) // เพิ่ม state สำหรับเจ้าหน้าที่พัสดุ
  const [selectedTeacher, setSelectedTeacher] = useState('')
  const [selectedOfficer, setSelectedOfficer] = useState('') // เพิ่ม state สำหรับผู้จ่ายพัสดุ
  const [selectedGrade, setSelectedGrade] = useState('')
  const [withdrawalDate, setWithdrawalDate] = useState(new Date().toISOString().slice(0, 10)) // เพิ่ม state วันที่เบิก
  ...
  useEffect(() => { 
    fetchData()
    fetchTeachers()
    fetchOfficers() // เพิ่มการ fetch เจ้าหน้าที่พัสดุ
    
    // ตั้งค่าผู้จ่ายพัสดุอัตโนมัติเป็น user ที่ login อยู่ (ถ้าเป็น staff หรือ admin)
  ...
  const fetchTeachers = async () => {
    ...
    if (error) {
      ...
      console.log('Teachers loaded:', data) // Debug log
      setTeachers(data || [])
    }
  }

  // ฟังก์ชันดึงรายชื่อเจ้าหน้าที่พัสดุ (admin และ staff)
  const fetchOfficers = async () => {
    ...
    if (error) {
      ...
      console.log('Officers loaded:', data) // Debug log
      setOfficers(data || [])
    }
  }
  ...
  const fetchAvailableBooks = async () => {
    ...
    try {
      ...
      // Filter ใน JavaScript เพื่อข้าม draft, pending, cancelled
  ...
  const fetchData = async () => {
    ...
    // ดึงรายการเบิก พร้อมข้อมูลผู้จ่ายพัสดุ
  ...
  // Toggle book selection with quantities
  ...
  // Update requested quantity for a book
  ...
  // Update approved quantity for a book
  ...
  // Handle Enter key to move to next approved quantity input
  const handleApprovedQtyKeyDown = (e, currentBookId) => {
    if (e.key === 'Enter') {
      e.preventDefault()
      
      // Get all selected book IDs in order
  ...
  // Copy requested to approved for all selected items
  ...
  const handleCreateNewWithdrawal = async () => {
    ...
    // สร้างเลขที่ใบเบิก
    const withdrawalNumber = `WD-${withdrawalDate.replace(/-/g, '')}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`
    ...
    const { data: wData, error: wError } = await supabase.from('withdrawals').insert({
      ...
      issued_by: selectedOfficer, // บันทึกผู้จ่ายพัสดุ
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4 grid grid-cols-2 gap-2 text-sm">
              ...
              <p><span className="text-gray-500">ผู้เบิก:</span> <span className="font-medium">{selectedWithdrawal.orders?.users?.full_name || '-'}</span></p>
      ...
      {/* New Withdrawal Modal - ปรับปรุงให้มีฟิลด์ผู้จ่ายพัสดุ */}
