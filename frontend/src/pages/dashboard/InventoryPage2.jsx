...
export default function InventoryPage2() {
    ...
    try {
      ...
      if (error) {
        ...
        Swal.fire({ 
          ...
          text: error.message,
          footer: 'กรุณาตรวจสอบ Foreign Key Relationship ระหว่าง books และ book_stock'
        })
      }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        ...
        // สร้าง book_stock เริ่มต้นสำหรับหนังสือใหม่
        if (newBook) {
          const { error: stockError } = await supabase.from('book_stock').insert({
            book_id: newBook.id,
  ...
  const handleCsvUpload = async (e) => {
    ...
    try {
      ...
      if (lines.length < 2) {
        Swal.fire({ 
          icon: 'warning', 
          title: 'ไฟล์ CSV ไม่ถูกต้อง', 
          text: 'ต้องมีหัวตารางและข้อมูลอย่างน้อย 1 แถว', 
      ...
      if (missing.length > 0) {
        Swal.fire({ 
          icon: 'error', 
          title: 'คอลัมน์ไม่ครบ', 
          text: `ต้องมีคอลัมน์: ${requiredCols.join(', ')}`, 
          confirmButtonColor: '#2563eb' 
        })
        return
      }
      ...
      // สร้าง book_stock สำหรับหนังสือที่นำเข้าทั้งหมด
      if (insertedBooks && insertedBooks.length > 0) {
        const stockEntries = insertedBooks.map(book => ({
          book_id: book.id,
          available_quantity: 0,
          min_quantity: 5
        }))
        
        const { error: stockError } = await supabase.from('book_stock').insert(stockEntries)
        if (stockError) {
      ...
      Swal.fire({ 
        icon: 'success', 
        title: 'นำเข้าสำเร็จ', 
        text: `เพิ่มหนังสือ ${rows.length} รายการ`, 
        timer: 2000, 
  ...
  // Helper function to get stock info - รองรับทั้ง array และ object
  const getStockInfo = (item) => {
    let stockData = null
    
    if (Array.isArray(item.book_stock) && item.book_stock.length > 0) {
      stockData = item.book_stock[0]
    } else if (item.book_stock && typeof item.book_stock === 'object') {
      stockData = item.book_stock
    }
    
    return {
      available: stockData?.available_quantity || 0,
      min: stockData?.min_quantity || 5
    }
  }
  ...
  const lowStock = items.filter(i => {
    const { available, min } = getStockInfo(i)
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-xl text-sm hover:bg-green-700 disabled:opacity-50"
          >
          ...
          <button 
            onClick={exportPdf} 
            className="flex items-center gap-2 px-4 py-2.5 border border-red-300 text-red-600 rounded-xl text-sm hover:bg-red-50"
          >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="kg3">อนุบาล 3</option>
