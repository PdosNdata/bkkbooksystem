...
const SettingDoc2 = () => {
  ...
  // Document Sequences State (for current fiscal year)
  ...
  const [currentFiscalYear, setCurrentFiscalYear] = useState(new Date().getFullYear() + 543);
  const [totalBudget, setTotalBudget] = useState('0');
  const [approvedOrdersTotal, setApprovedOrdersTotal] = useState(0);
  ...
  const loadSequences = async () => {
    try {
      // แปลงปี พ.ศ. เป็น ค.ศ. สำหรับค้นหาใน database
      const gregorianYear = currentFiscalYear - 543;
  ...
  const loadFiscalYearSettings = async () => {
    try {
      console.log('📖 Loading fiscal year settings...');
      
      // โหลดการตั้งค่าปีงบประมาณจาก system_settings
      const { data, error } = await supabase
      ...
      if (data && data.value) {
        ...
        setFiscalYearStartDate(settings.start_date || '');
        setFiscalYearEndDate(settings.end_date || '');
        setCurrentFiscalYear(settings.current_year || new Date().getFullYear() + 543);
        setTotalBudget(settings.total_budget || '0');
      } else {
        console.log('⚠️ No settings found, using defaults');
        // ตั้งค่าเริ่มต้น
        const currentYear = new Date().getFullYear() + 543;
        setCurrentFiscalYear(currentYear);
        setFiscalYearStartDate(`${currentYear - 543}-10-01`); // 1 ตุลาคม
        setFiscalYearEndDate(`${currentYear - 542}-09-30`); // 30 กันยายน ปีถัดไป
        setTotalBudget('0');
      }

      // โหลดยอดรวมจากคำสั่งซื้อที่อนุมัติแล้ว
      await loadApprovedOrdersTotal();
    } catch (error) {
      console.error('💥 Error loading fiscal year settings:', error);
      // ตั้งค่าเริ่มต้น
      const currentYear = new Date().getFullYear() + 543;
      setCurrentFiscalYear(currentYear);
      setFiscalYearStartDate(`${currentYear - 543}-10-01`);
      setFiscalYearEndDate(`${currentYear - 542}-09-30`);
      setTotalBudget('0');
    }
  };
  ...
  const handleSaveFiscalYear = async () => {
    ...
    if (!fiscalYearStartDate) {
      showMessage('error', 'กรุณาระบุวันที่เริ่มต้นปีงบประมาณ');
      return;
    }

    if (!fiscalYearEndDate) {
      showMessage('error', 'กรุณาระบุวันที่สิ้นสุดปีงบประมาณ');
      return;
    }

    if (!currentFiscalYear || currentFiscalYear < 2500 || currentFiscalYear > 2600) {
      showMessage('error', 'กรุณาระบุปีงบประมาณที่ถูกต้อง (2500-2600)');
      return;
    }
    ...
    if (startDate >= endDate) {
      showMessage('error', 'วันที่เริ่มต้นต้องอยู่ก่อนวันที่สิ้นสุด');
      return;
    }
    ...
    try {
      ...
      console.log('💾 Saving fiscal year settings:', settings);
      ...
      if (existingData) {
        ...
        console.log('✅ Updated existing settings:', result);
      } else {
        // สร้างข้อมูลใหม่
        result = await supabase
          .from('system_settings')
        ...
        console.log('✅ Created new settings:', result);
      }
      ...
      showMessage('error', `ไม่สามารถบันทึกการตั้งค่าได้: ${error.message}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const generatePreview = (docType, number) => {
    return `${docType.prefix}${String(number).padStart(3, '0')}/${currentFiscalYear}`;
  };
        ...
        <TabsContent value="sequences" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                กำหนดเลขเริ่มต้นและรีเซ็ตเลขที่เอกสารสำหรับปีงบประมาณ {currentFiscalYear}
              </CardDescription>
            </CardHeader>
            <CardContent>
