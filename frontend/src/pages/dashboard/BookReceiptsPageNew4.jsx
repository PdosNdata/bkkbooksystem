...
export default function BookReceiptsPageNew4() {
  ...
  const generateReceiptNumber = async () => {
    try {
      ...
      if (docTypeError || !docType) {
        ...
        return `REC-${receiptDate.replace(/-/g, '')}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`
      }
      ...
      // สร้างเลขที่เอกสาร: PREFIX + ปี(4หลัก) + เลขลำดับ(4หลัก)
      // เช่น: REC-2568-0001
      const receiptNumber = `${docType.prefix}-${currentYear + 543}-${nextNumber.toString().padStart(4, '0')}`
